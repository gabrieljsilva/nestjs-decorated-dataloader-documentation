					...
					<div className="space-y-8">
						...
						<section id="quick-start" className="space-y-4">
							...
							<div className="space-y-4">
								...
								<p>
									Configure the <code>DataloaderModule</code> in your
									application module:
								</p>
								<CodeBlock
    ...
    return this.dataloaderService.load({ from: UserEntity, field: "photos", data: user });
						...
						<section id="advanced-concepts" className="space-y-8">
							...
							<div id="handling-circular-dependencies" className="space-y-4">
								...
								<p>
									Circular dependencies between entities (e.g., User ↔ Photo)
									can cause metadata resolution errors when using
									reflect-metadata. For example:
								</p>
								<ul className="list-disc pl-6 space-y-2">
									<li>
...
class Photo {
  user: Relation<User>;
								...
								<ul className="list-disc pl-6 space-y-2">
									<li>
										<strong>Generic Type Erasure:</strong> reflect-metadata
										cannot infer generic types like{" "}
										<code>Relation&lt;Photo&gt;</code>, so it defaults the
										metadata to undefined, avoiding circular resolution errors.
									</li>
									<li>
										<strong>Explicit Type Declaration:</strong> You must
										manually specify the wrapped type (e.g.,{" "}
								...
								<div className="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4 my-4">
									...
									<ul className="list-disc pl-6 mt-2">
										<li>
											...
											dependencies. For non-circular references, use direct
											types (e.g., <code>Photo</code> instead of{" "}
							...
							<div id="aliases" className="space-y-4">
								...
								<p>
									...
									class, which is especially useful when working with more
									complex architectures that include abstract or shared classes.
								</p>
								...
								<div className="bg-blue-100 border-l-4 border-blue-500 text-blue-700 p-4 my-4">
									...
									<p>
										...
										abstract class that doesn't allow decorators. Aliases
										provide a way to assign a handler to such cases.
									</p>
								</div>
...
export class ConcretePhotoService {}`}
								/>
								<p>
									This allows <code>PhotoService</code> to serve as the
									dataloader handler for <code>AbstractPhotoService</code>.
								</p>
							</div>
							...
							<div id="under-the-hood" className="space-y-4">
								...
								<p>
									<code>nestjs-decorated-dataloaders</code> is built on top of
									the GraphQL Dataloader library. At its core, a dataloader is a
								...
								<ul className="list-disc pl-6 space-y-2">
									<li>
										...
										ensures that, rather than issuing one query per entity
										(e.g., fetching one photo per user), the dataloader combines
										them into a single query that fetches all the photos for the
										users in one go.
									</li>
									<li>
										<strong>Caching:</strong> Dataloader caches query results,
										preventing redundant queries for the same data within the
										same request cycle. This ensures that once a resource is
										fetched, subsequent requests for the same resource will use
										the cached data.
									</li>
								</ul>
							</div>
							...
							<div id="high-level-abstraction" className="space-y-4">
								...
								<p>
									...
									dataloader setup, making it simple to handle related entities
									in GraphQL resolvers without manual dataloader instantiation
